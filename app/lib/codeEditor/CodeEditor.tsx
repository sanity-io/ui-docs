...
export function CodeEditor(
  ...
  const editorThemeExtension = useMemo(
    () => getEditorThemeExtension({theme: rootTheme.theme.v2!, tone: rootTheme.tone}),
    [rootTheme],
  )
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      if (isHotkey(['mod', 's'], event.nativeEvent)) {
        ...
        async function run() {
          const anchorResult = await runPrettier({code: value, cursorOffset: selection.anchor})
          const focusResult = await runPrettier({code: value, cursorOffset: selection.focus})
